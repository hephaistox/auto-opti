(ns auto-opti.time-based.impl.aggregator-item-test
  (:require
   #?(:clj [clojure.test :refer [deftest is]]
      :cljs [cljs.test :refer [deftest is] :include-macros true])
   [auto-opti                                 :as-alias opti]
   [auto-opti.time-based.impl.aggregator-item :as sut]))
...
(deftest bucket-to-bucket-aggregate-test
  (is (= [30 30 31 31 32 32]
         (sut/bucket-to-bucket-aggregate #::opti{:start-bucket 10
                                                 :end-bucket 16
                                                 :step 2
                                                 :start-bucket-aggregate 30
                                                 :end-bucket-aggregate 33})))
  (is (empty? (sut/bucket-to-bucket-aggregate #::opti{:start-bucket 10
                                                      :end-bucket 10
                                                      :step 2
                                                      :start-bucket-aggregate 30
...
(deftest concerns-bucket-test
  ...
  (is
   (sut/concerns-bucket? 30 #::opti{:start-bucket 3})
...
(deftest concerns-bucket-aggregate-test
  ...
  (is
   (sut/concerns-bucket-aggregate? 30 #::opti{:start-bucket-aggregate 3})
